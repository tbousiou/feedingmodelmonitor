    ...
    if selected_model_index == 0:
        ...
        **Click on +/- to change the model parameteres** to match the user curve somewhere between the top and bottomm percentiles""")
        ...
        parameter_a = st.number_input(
            'Parameter a', min_value=1.0, max_value=20.0, value=15.36, step=0.5)
        parameter_b = st.number_input(
            'Parameter b', min_value=0.001, max_value=0.003, value=0.0022, step=0.0001, format='%.4f')
        base_model = model1()
        user_model = model1("User model from base 1",
                            a=parameter_a, b=parameter_b)

        estimator_dmi = estimator_bw.apply(lambda x: base_model.dmicalc(x))
        estimator_dmi['user'] = estimator_bw['user'].apply(
            lambda x: user_model.dmicalc(x))
        # estimator_dmi
